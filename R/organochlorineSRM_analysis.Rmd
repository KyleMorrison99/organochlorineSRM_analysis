...
im <- im %>%
  ...
  mutate(impact_broad = case_when(
    ...
    impact %in% c("diabetes", "thyroid function", "hypertension", "endometriosis") ~ "Endocrine disrupting",
...
ggplot(ocp_im_summary_filtered, 
       aes(x = fct_rev(fct_reorder(impact_broad, count, .fun = 'sum')),
...
## Figure s9 - Total count of each database used to search the literature within each meta-analysis
```{r}
