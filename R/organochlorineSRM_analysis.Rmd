...
ocp_count <-
 ...
 filter(ocp != "not reported") %>% 
  arrange(desc(n)) %>% 
  mutate(ocp = ifelse(n <= 5, "other", as.character(ocp))) %>% 
...
# Preprocess dataset to count frequencies
df <- df %>%
  group_by(ocp, subject, impact) %>%
  summarise(freq = n()) %>%
  ungroup()

# Filter out ocp with count <= 5
df_filtered <- df %>%
  group_by(ocp) %>%
  filter(sum(freq) > 5)
...
# Define a vector with the desired order of the subjects
subject_order <- c("environment", "non-human animal", "human")  

# Transform 'subject' into an ordered factor
df_filtered1$subject <- factor(df_filtered1$subject, levels = subject_order, ordered = TRUE)

# Create alluvial plot
ggplot(data = df_filtered1,
       aes(axis1 = ocp, axis2 = subject, axis3 = impact, y = freq)) +
  scale_x_discrete(limits = c("OCP", "Subject", "Impact"), expand = c(.1, .35)) +
  xlab("Variables") +
  geom_alluvium(aes(fill = subject)) +
  geom_stratum(width = 7/10, fill = "white", color = "black") +
