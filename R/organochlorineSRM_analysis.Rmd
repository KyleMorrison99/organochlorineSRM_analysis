...
fig1 <- msp %>%
  ...
  geom_line(color = "#007030", size = 1) +
  geom_point(shape = 21, size = 3, fill = "#8FBF43", color = "black", stroke = 0.5, alpha = 0.7) +
  scale_x_continuous(breaks = seq(min(msp$publication_year), max(msp$publication_year), by = 2)) +
  theme_minimal() +
...
heterogeneity_count <-  ## REPLACE TAU SQUARED IN DATASET
  sd %>% 
...
fields <- Publication_info_discipline %>%
  mutate(title = tolower(unlist(lapply(data.frame(t(str_split_fixed(paper_title, " ", n = 15)[,1:14])), 
                                       function(x) str_c(x, collapse  = " ")))),
         ntitle = paste(tolower(str_split_fixed(study_id, "\\_", n = 2)[,1]), title, sep = " "),
...
'' fig 5i - collaborations between pesticide ch
```{r}
fig5 <- ((fig5e | fig5f) /(fig5g  | fig5h))
fig5
```
