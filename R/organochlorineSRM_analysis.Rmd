---
title: "Sixty years since Silent Spring: a map of meta-analyses on organochlorine pesticides reveals urgent needs for improving methodological quality"
...
sd_bib_alt <- sd %>%
  ...
  filter(!is.na(CEESAT2_1.1))

# Converting policy to binary and dropping NAs
sd_bib_alt_policy <- sd_bib_alt %>%
...
policy <- sd_bib_alt_policy$total_points[sd_bib_alt_policy$binary_policy == "Cited in Policy"]
not_policy <- sd_bib_alt_policy$total_points[sd_bib_alt_policy$binary_policy == "Not Cited in Policy"]


# Create a tibble for policy values
policy_df <- tibble(
  values = policy,
  category = "Cited in Policy"
)

# Create a tibble for not_policy values
not_policy_df <- tibble(
  values = not_policy,
  category = "Not Cited in Policy"
)
...
t.test(policy,not_policy) # Can you explain why a t-test does not fit this data 


glm(values~category,
                 family = "poisson", data=policy_combined)


glm.nb(values~category, data=policy_combined)
...
```{r, fig.width=25, fig.height=15}
sd_bib_alt_guideline <- sd_bib_alt %>% 
...
figs5 <- ggplot(data = percent_ceesat_score2, aes(x = question, y = percent, fill = score)) +
  geom_col(width = 0.7, position = "fill", color = "black") +
  geom_text(aes(label = n), position = position_fill(vjust = 0.5), size = 7, fontface = "bold") +
  coord_flip() +
  guides(fill = guide_legend(reverse = TRUE)) +
...
figs5
# ggsave(here("figures", "figs5.pdf"), width = 25, height = 15, units = "cm", scale = 2, dpi = 800)
# ggsave(here("figures", "figs5.jpg"), width = 25, height = 15, units = "cm", scale = 2, dpi = 800)

guidline <- sd_bib_alt_guideline$total_points[sd_bib_alt_guideline$binary_guideline == "Reported"]
no_guideline <- sd_bib_alt_guideline$total_points[sd_bib_alt_guideline$binary_guideline == "Not reported"]

# QQ Plot
 qqnorm(guidline)
 qqline(guidline, col = "red")

# QQ Plot
 qqnorm(no_guideline)
 qqline(no_guideline, col = "red")

t.test(guidline,no_guideline)
...
# ggsave(here("figures", "figs6.pdf"), width = 25, height = 15, units = "cm", scale = 2, dpi = 800)
# ggsave(here("figures", "figs6.jpg"), width = 25, height = 15, units = "cm", scale = 2, dpi = 800)
```
...
# ggsave(here("figures", "figs7.pdf"), width = 25, height = 15, units = "cm", scale = 2, dpi = 800)
# ggsave(here("figures", "figs7.jpg"), width = 25, height = 15, units = "cm", scale = 2, dpi = 800)
```
...
fig3a <- bias_method_count %>%
  ggplot(aes(x = n, y = reorder(bias_assessment_method, n), fill = ifelse(grepl("Not reported", bias_assessment_method), "#7370b3", "#1b9e77"))) +
  geom_bar(stat = "identity", width = 0.8 , alpha = 0.7) +
  geom_text(aes(label = n, x = n / 2, y = reorder(bias_assessment_method, n)), hjust = 0.5, size = 12, color = "black") +
...
# ggsave(here("figures", "fig3h.pdf"), width = 16, height = 10, units = "cm", scale = 2, dpi = 800)
# ggsave(here("figures", "fig3h.jpg"), width = 16, height = 10, units = "cm", scale = 2, dpi = 800)
```
...
figs8 <- bias_assessment_alluvial %>% 
...
fig3b <- heterogeneity_count %>%
  ggplot(aes(x = n, y = reorder(heterogeneity_assessment_method, n), fill = ifelse(grepl("Not reported", heterogeneity_assessment_method), "#7370b3", "#1b9e77"))) +
  geom_bar(stat = "identity", width = 0.8 , alpha = 0.7) +
  geom_text(aes(label = n, x = n / 2, y = reorder(heterogeneity_assessment_method, n)), hjust = 0.5, size = 12, color = "black") +
...
# ggsave(here("figures", "fig3b.jpg"), width = 16, height = 10, units = "cm", scale = 2, dpi = 800)
...
fig3c <- sensitivity_count %>%
  ggplot(aes(x = n, y = reorder(sensitivity_analysis_method, n), fill = ifelse(grepl("Not reported", sensitivity_analysis_method), "#7370b3", "#1b9e77"))) +
  geom_bar(stat = "identity", width = 0.8 , alpha = 0.7) +
  geom_text(aes(label = n, x = n / 2, y = reorder(sensitivity_analysis_method, n)), hjust = 0.5, size = 12, color = "black") +
...
# ggsave(here("figures", "fig3c.jpg"), width = 16, height = 10, units = "cm", scale = 2, dpi = 800)
...
# Create the Alluvial plot
figs10 <- sensitivity_analysis_alluvial %>% 
...
fig3d <- reporting_guide_count %>%
  ggplot(aes(x = n, y = reorder(reporting_standards_type, n), fill = ifelse(grepl("Not reported", reporting_standards_type), "#7370b3", "#1b9e77"))) +
  geom_bar(stat = "identity", width = 0.8 , alpha = 0.7) +
  geom_text(aes(label = n, x = n / 2, y = reorder(reporting_standards_type, n)), hjust = 0.5, size = 12, color = "black") +
 ...
 scale_x_continuous(expand = c(0, 0), limits = c(0, max(reporting_guide_count$n)*1.3)) +
  labs(title = "Reporting Guideline Count", x = NULL, y = NULL, size = 30) +
  organochlorTHEME() 

fig3d

# ggsave(here("figures", "fig3d.pdf"), width = 16, height = 10, units = "cm", scale = 2, dpi = 800)
# ggsave(here("figures", "fig3d.jpg"), width = 16, height = 10, units = "cm", scale = 2, dpi = 800)
```
...
# Create the Alluvial plot
figs11 <- reporting_standards_alluvial %>% 
...
## Figure 3
```{r, fig.width=36, fig.height=20}

fig3 <- fig3a + fig3b + fig3c + fig3d +
        plot_layout(ncol = 2, nrow = 2)
fig3

# ggsave(here("figures", "fig3.pdf"), width = 36 , height = 20 , units = "cm", scale = 2, dpi = 400)
# ggsave(here("figures", "fig3.jpg"), width = 36 , height = 20, units = "cm", scale = 2, dpi = 400)
```
...
figs15 <- software_analysis_alluvial %>% 
...
rob_method_count <- sd %>% 
  ...
  mutate(rob_assessment_method = ifelse(grepl("Newcastle Ottawa scale", rob_assessment_method), "Newcastle Ottawa\nscale", rob_assessment_method)) %>%
...
# Create the count plot
figs20 <- rob_method_count %>%
  ggplot(aes(x = n, y = reorder(rob_assessment_method, n), fill = ifelse(grepl("Not reported", rob_assessment_method), "#7370b3", "#1b9e77"))) +
  geom_bar(stat = "identity", width = 0.8 , alpha = 0.7) +
  geom_text(aes(label = n, x = n / 2, y = reorder(rob_assessment_method, n)), hjust = 0.5, size = 12, color = "black") +
...
# Create the count plot
figs22 <- bias_visualization_count %>%
  ggplot(aes(x = n, y = reorder(bias_assessment_visualization, n), fill = ifelse(grepl("Not reported", bias_assessment_visualization), "#7370b3", "#1b9e77"))) +
  geom_bar(stat = "identity", width = 0.8 , alpha = 0.7) +
  geom_text(aes(label = n, x = n / 2, y = reorder(bias_assessment_visualization, n)), hjust = 0.5, size = 12, color = "black") +
...
# Create the Alluvial plot
figs23 <- bias_vizualisation_alluvial %>% 
...
figs24 <- ggraph(mygraph, layout = 'circlepack', weight = size) + 
...
ggsave(here("figures", "figs24.pdf"), width = 18, height = 15, units = "cm", scale = 2, dpi = 800)
ggsave(here("figures", "figs24.jpg"), width = 18, height = 15, units = "cm", scale = 2, dpi = 800)
```
...
figs25 <- ocp_count %>%
  ...
  geom_bar(stat = "identity", width = 0.8, alpha = 0.7) +
  geom_text(aes(label = n, x = n / 2, y = reorder(ocp, n)), hjust = 0.5, size = 7, color = "black") +
...
subject_count <- sub %>% 
  ...
  mutate(subject = ifelse(grepl("Non-human animal", subject, ignore.case = TRUE), "Non-human\nanimal", subject)) %>% 
  count(subject)
...
figs26 <- subject_count %>%
  ggplot(aes(x = n, y = reorder(subject, n), fill = "#1b9e77")) +
  geom_bar(stat = "identity", width = 0.8 , alpha = 0.7) +
  geom_text(aes(label = n, x = n / 2, y = reorder(subject, n)), hjust = 0.5, size = 12, color = "black") +
...
figs27 <- impact_count %>%
  ...
  geom_bar(stat = "identity", width = 0.8 , alpha = 0.7) +
  geom_text(aes(label = n, x = n / 2, y = reorder(impact, n)), hjust = 0.5, size = 10, color = "black") +
...
# Create the count plot for impacts 
figs28 <- impact_count_broad %>%
  ggplot(aes(x = n, y = reorder(impact_broad, n), fill = "#1b9e77")) +
  geom_bar(stat = "identity", width = 0.8 , alpha = 0.7) +
  geom_text(aes(label = n, x = n / 2, y = reorder(impact_broad, n)), hjust = 0.5, size = 10, color = "black") +
...
fig4
 ...
 ggsave(here("figures", "fig4.jpg"), width = 40, height = 25, units = "cm", scale = 2, dpi = 800)
...
## Figure s30, s31, s32, s33 & s34
```{r,fig.width=16, fig.height=10}

bibliometric_analysis <- biblioAnalysis(bib_sco)
par(cex=1.5)
plot(bibliometric_analysisbibliometric_analysis)
...
```{r,fig.width=16, fig.height=10}
figs30_data <- bibliometric_analysis$CountryCollaboration
figs30_tibble <- as_tibble(figs33_data)

figs30_tibble_long <- figs30_tibble %>% 
                      pivot_longer(cols = c(SCP, MCP), names_to = "Country_Collaboration", values_to = "value")


# Summarizing and filtering for top ten countries
top_countries <- figs30_tibble_long %>%
  group_by(Country) %>%
  summarize(Total = sum(value)) %>%
  arrange(desc(Total)) %>%
  top_n(10, Total)

# Filtering the original dataset to keep only the top ten countries
figs30_tibble_long_filtered <- figs30_tibble_long %>%
  filter(Country %in% top_countries$Country)


figs30 <- ggplot(figs30_tibble_long_filtered, aes(fill = Country_Collaboration, y = value, x = Country)) +
...
figs31_data <- bibliometric_analysis$Authors
figs31_tibble <- as_tibble(figs34_data)

figs31_tibble_filtered <- figs34_tibble %>% 
                          slice_head(n=10) 
  

figs31 <- ggplot(figs31_tibble_filtered, aes(y = n, x = reorder(AU,n))) +
          geom_bar(stat = "identity", width = 0.8 , alpha = 0.7, fill = "#1b9e77") +
          geom_text(aes(label = n, y = n/2), hjust = 0.5, color = "black", size = 10) +
          coord_flip() +
          organochlorTHEME()

# ggsave(here("figures", "figs31.pdf"), width = 16, height = 10, units = "cm", scale = 2, dpi = 800)
# ggsave(here("figures", "figs31.jpg"), width = 16, height = 10, units = "cm", scale = 2, dpi = 800) 
...
# ggsave(here("figures", "figs38.pdf"), width = 16, height = 12, units = "cm", scale = 2, dpi = 800)
# ggsave(here("figures", "figs38.jpg"), width = 16, height = 12, units = "cm", scale = 2, dpi = 800)
```
...
jpeg(filename = here("figures", "figs39.jpg"), width = 800, height = 800)
...
```{r,fig.width= 12, fig.height=12}
jpeg(filename = here("figures", "figs40.jpg"), width = 800, height = 800)

diag(NetMatrix_country) <- 0

# Create a chord diagram of the network matrix
figs43 <- chordDiagram(NetMatrix_country, annotationTrack = "grid", preAllocateTracks = 1)
...
colnames(NetMatrix_continent)[colnames(NetMatrix_continent) == "Australia"] <- "Other"
rownames(NetMatrix_continent)[rownames(NetMatrix_continent) == "Australia"] <- "Other"

colnames(NetMatrix_continent)[colnames(NetMatrix_continent) == "Brazil"] <- "Other"
rownames(NetMatrix_continent)[rownames(NetMatrix_continent) == "Brazil"] <- "Other"
...
colnames(NetMatrix_continent)[colnames(NetMatrix_continent) == "Egypt"] <- "Other"
rownames(NetMatrix_continent)[rownames(NetMatrix_continent) == "Egypt"] <- "Other"
...
jpeg("figures//figs42.jpeg", width = 16, height = 16, units = "cm", res = 300)

# Create a chord diagram of the network matrix
fig6 <- chordDiagram(small_matrix, 
...
circos.trackPlotRegion(track.index = 1, panel.fun = function(x, y) {
  ...
  ylim = get.cell.meta.data("ylim")
  sector.name = get.cell.meta.data("sector.index")
  circos.text(mean(xlim), ylim[1], sector.name, facing = "bending.inside", niceFacing = TRUE, adj = c(0.5, 1.75), col = "white", cex = 1.2)
}, bg.border = NA)
...
fields <- sd %>%
  mutate(
    ...
    Journal_Category_Allocated_Broad = str_replace_all(Journal_Category_Allocated_Broad, "Toxicology", "Other"),
    Journal_Category_Allocated_Broad = str_replace_all(Journal_Category_Allocated_Broad, "Multidisciplinary", "Multi\ndisciplinary"),
    Journal_Category_Allocated_Broad = str_replace_all(Journal_Category_Allocated_Broad, "Marine Biology", "Other"),
    Journal_Category_Allocated_Broad = str_replace_all(Journal_Category_Allocated_Broad, "Environmental Science", "Environemental\nScience"),    Journal_Category_Allocated_Broad = str_replace_all(Journal_Category_Allocated_Broad, "Psychology", "Other")
  ) %>% 
...
# Define colors
my_colors2 <- c("#1B9E77", "#D95F02", "#7570B3",  "#E6AB02")

my_colors2 <- rep(my_colors2, length.out = ncol(small_matrix))

jpeg("figures//figs42.jpeg", width = 20, height = 20, units = "cm", res = 300)

# Create a chord diagram with specified colors
figs47 <- chordDiagram(small_matrix, 
                       annotationTrack = "grid", 
                       annotationTrackHeight = mm_h(7), 
                       preAllocateTracks = 1,
                       grid.col = my_colors2)
...
circos.trackPlotRegion(track.index = 1, panel.fun = function(x, y) {
  ...
  ylim = get.cell.meta.data("ylim")
  sector.name = get.cell.meta.data("sector.index")
  circos.text(mean(xlim), ylim[1] , sector.name, facing = "clockwise", niceFacing = TRUE, adj = c(-0.1, 0.5), col = "black", cex = 1)
}, bg.border = NA)
