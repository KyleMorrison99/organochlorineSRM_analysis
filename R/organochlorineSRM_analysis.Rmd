...
fig3e <- heterogeneity_count %>%
  ...
  labs(title = "Heterogeneity Count", x = NULL, y = NULL, tag = "A", size = 30) +
  theme_suppl()
...
fig3f <- sensitivity_count %>%
  ...
  scale_fill_identity(guide = "none") +
  scale_x_continuous(expand = c(0, 0), limits = c(0, max(visualization_count$n)*1.3)) +
  labs(title = "Sensitivity Analysis Count", x = NULL, y = NULL, tag = "B", size = 30) +
  theme_suppl()
...
fig3h <- bias_method_count %>%
  ...
  labs(title = "Publication Bias Count", x = NULL, y = NULL, tag = "D", size = 30) +
  theme_suppl()
...
## Figure 3
```{r}
fig3 <- fig3e + fig3f + fig3g + fig3h + fig3i + fig3j +
        plot_layout(ncol = 2, nrow = 3)
fig3


 ggsave(here("figures", "fig3.pdf"), width = 36 , height = 30 , units = "cm", scale = 2, dpi = 400)
 ggsave(here("figures", "fig3.jpg"), width = 36 , height = 30, units = "cm", scale = 2, dpi = 400)
...
# Assuming you want to start with the green and purple from the Dark2 palette
dark2_colors <- brewer.pal(8, "Dark2")
custom_colors <- c(dark2_colors[4], dark2_colors[3], dark2_colors[-c(3, 4)])  # Green and Purple first
...
fig4 <- ocp_im_summary_filtered %>% 
  ...
  scale_fill_manual(values = custom_colors) +  # Use custom color order
  labs(x = "Impact",
