...
fig1 <- sd1 %>% 
  ...
  # Add the non-cumulative count as a bar plot
  geom_col(aes(y = n), fill = "#A65475", alpha = 0.7) +
  
  # Add the cumulative count as a line plot
  geom_line(aes(y = n_cumulative), color = "#54A76A", size = 1, linetype = "solid") +
  
  # Highlight points on the line
  geom_point(aes(y = n_cumulative), shape = 21, size = 3, fill = "#54A76A", stroke = 0) +
  
  # Set the x-axis breaks
  scale_x_continuous(breaks = seq(min(sd1$publication_year), max(sd1$publication_year), by = 2)) +
  
  # Set the y-axis scales and labels
  scale_y_continuous("Cumulative Article Count", sec.axis = sec_axis(~.*max_scale, name = "Article Count")) +
  ...
  geom_text(aes(label = n), size = 4, nudge_y = 4, color = "#666666", fontface = "bold")
...
```{r}
# Join 'sd' and 'sub' data frames by 'study_id'
sd_sub <- left_join(sd, sub, by = "study_id")

sd_sub %>%
  # Split 'subject' column by comma followed by one or more spaces
  mutate(subjects = strsplit(subject, ",\\s+")) %>%
  # Expand list column 'subjects' into multiple rows
  unnest(subjects) %>%
  # Count number of articles by 'publication_year' and 'subjects'
  count(publication_year, subjects) %>%
  # Reorder 'subjects' by count 'n'
  group_by(subjects = reorder(subjects, n)) %>%
  # Compute cumulative count of articles
  mutate(n_cumulative = cumsum(n)) %>%
  # Create a plot
  ggplot(aes(x = publication_year, y = n_cumulative, fill = subjects)) +
  # Add a filled area plot
  geom_area(size = 1.2, alpha = 0.7) +
  # Set the x-axis breaks
  scale_x_continuous(breaks = seq(min(sd_sub$publication_year), max(sd_sub$publication_year), by = 2)) +
  # Set the plot theme
  theme_minimal() +
  ...
  theme(panel.grid.major.y = element_line(color = "gray", linetype = "dashed"),
        ...
        plot.subtitle = element_text(size = 14),
        plot.caption = element_text(size = 10, hjust = 0)) +
  # Set the fill color palette and label
